...
CREATE TABLE IF NOT EXISTS products_spare_parts (
    ...
    description TEXT,
    brand_id INTEGER REFERENCES vehicle_brands(brand_id),
...
CREATE TABLE IF NOT EXISTS purchase_orders_parts (
    ...
    po_type VARCHAR(20) NOT NULL DEFAULT 'regular' CHECK (po_type IN ('regular','urgent','blanket','contract')),
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'draft' CHECK (status IN ('draft','sent','acknowledged','partial_received','received','completed','cancelled')),
    payment_terms VARCHAR(20) NOT NULL DEFAULT 'net_30' CHECK (payment_terms IN ('cod','net_30','net_60','advance')),
...
CREATE TABLE IF NOT EXISTS purchase_order_details (
    ...
    line_status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (line_status IN ('pending','partial','received','cancelled')),
    item_notes TEXT
);`
...
CREATE TABLE IF NOT EXISTS goods_receipts (
    ...
    receipt_status VARCHAR(20) NOT NULL DEFAULT 'partial' CHECK (receipt_status IN ('partial','complete','with_discrepancy')),
    receipt_notes TEXT,
...
CREATE TABLE IF NOT EXISTS goods_receipt_details (
    ...
    unit_cost DECIMAL(15,2) NOT NULL CHECK (unit_cost >= 0),
    total_cost DECIMAL(15,2) NOT NULL CHECK (total_cost >= 0),
    condition_received VARCHAR(20) NOT NULL DEFAULT 'good' CHECK (condition_received IN ('good','damaged','expired','wrong_item')),
...
CREATE TABLE IF NOT EXISTS supplier_payments (
    ...
    due_date TIMESTAMP,
    payment_method VARCHAR(20) NOT NULL DEFAULT 'transfer' CHECK (payment_method IN ('cash','transfer','check','credit')),
    payment_reference VARCHAR(100),
    invoice_number VARCHAR(100),
    payment_status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (payment_status IN ('pending','partial','paid','overdue','disputed')),
...
CREATE INDEX IF NOT EXISTS idx_products_spare_parts_location ON products_spare_parts(location_rack);
CREATE INDEX IF NOT EXISTS idx_products_spare_parts_stock_level ON products_spare_parts(stock_quantity);
...
CREATE INDEX IF NOT EXISTS idx_purchase_orders_parts_date ON purchase_orders_parts(po_date);
CREATE INDEX IF NOT EXISTS idx_purchase_orders_parts_status ON purchase_orders_parts(status);
CREATE INDEX IF NOT EXISTS idx_purchase_orders_parts_type ON purchase_orders_parts(po_type);
...
CREATE INDEX IF NOT EXISTS idx_purchase_order_details_status ON purchase_order_details(line_status);
